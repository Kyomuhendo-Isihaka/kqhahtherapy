                        ...
                        <div class="card-body">
                            <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
                                <div class="p-4 rounded-md lg:col-span-2 bg-slate-100 dark:bg-zink-600">
                                    <img src="{{ asset('storage/'.$ct->image_path) }}" alt="" style="height: 100px; width:100%">
                                </div><!--end col-->
                                <div class="flex flex-col lg:col-span-4">
                                    <div>
                                        <h5 class="mb-1 text-16"><a href="" class="nav-link">{{ $ct->name }}</a></h5>
                                    </div>
                                    <div class="flex items-center gap-2 mt-auto">
                                        <div class="inline-flex p-2 text-center border rounded input-step border-slate-200 dark:border-zink-500">
                                            ...
                                            <form action="{{ route('cart.reduce') }}" method="post">
                                                @csrf
                                                <input type="number" name="product_id" value="{{ $ct->id }}" hidden>
                                                <input type="number" name="user_id" value="1" hidden>
                                                <input type="hidden" name="action" value="decrease">
                                                <button type="submit" class="border w-7 leading-[15px] minus-value bg-slate-200 dark:bg-zink-600 dark:border-zink-600 rounded transition-all duration-200 ease-linear border-slate-200 text-slate-500 dark:text-zink-200 hover:bg-custom-500 dark:hover:bg-custom-500 hover:text-custom-50 dark:hover:text-custom-50 hover:border-custom-500 dark:hover:border-custom-500 focus:bg-custom-500 dark:focus:bg-custom-500 focus:border-custom-500 dark:focus:border-custom-500 focus:text-custom-50 dark:focus:text-custom-50"><i data-lucide="minus" class="inline-block w-4 h-4"></i></button>
                                            </form>

                                            <input type="number" class="text-center ltr:pl-2 rtl:pr-2 w-15 h-7 products-quantity dark:bg-zink-700 focus:shadow-none" value="{{ $ct->no_of_items }}" min="0" max="100" readonly="">
                                ...
                                <div class="flex justify-between w-full lg:flex-col lg:col-end-13 lg:col-span-2">
                                    ...
                                    <h6 class="mt-auto text-16 ltr:lg:text-right rtl:lg:text-left">$<span class="products-line-price">{{ $ct->pdt_cost }} </span></h6>
                                </div><!--end col-->
                            </div><!--end grid-->
                        </div>
                    </div><!--end card-->
